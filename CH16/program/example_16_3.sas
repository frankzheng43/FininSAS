...
data final;
   merge bench ret;by code y m;
      ...
      if mvrank=. or mbrank=. or ret=. then delete;
run;
proc sort data=final;by month mvrank mbrank;
run;
proc means data=final noprint;
   var ret;
   weight mv;
   by month mvrank mbrank;
...
data benchret;
   set benchret;
      bench=mvrank||mbrank;
      /*
       if using three different sorting, new=a||b||c could combine three variable into one variable
	 */
run;
...
%macro h(file,code,time,ret,p,h);
   ...
   /*
   revise 20150914
   we only ues the one variable to descending sort
   */
   data &file;
      set &file;
         %do i=1 %to &h;
